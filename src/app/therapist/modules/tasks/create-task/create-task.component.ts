...
import {
  ApiResponseMyVideosI,
  GetAllMyVideosI,
} from 'src/app/therapist/interfaces/videos.interface';
import {
  ApiResponseCategoriesI,
  GetCategoryI,
} from 'src/app/therapist/interfaces/categories.interface';
import {
  ApiResponseRegisterTaskDetailI,
  RegisterTaskDetailI,
...
@Component({
  ...
  styleUrls: [
    ...
    './../list-my-tasks/list-my-tasks.component.css',
  ],
})
export class CreateTaskComponent {
  ...
  getAllMyVideos() {
    ...
    this.videosService
      .getAllMyVideos(this.headers.getHeaders(), true)
      .subscribe({
  ...
  getInfoFormUploadTask() {
    let formUploadTask: RegisterTaskDetailI = {
      ...
      isPublic:
        this.uploadTaskForm.get('visibility')?.value === 'public'
          ? true
          : false,
  ...
  registerTaskDetail() {
    ...
    if (this.arrayVideosId.length != 0) {
      this.tasksService
        .registerTaskDetailWithVideos(
          this.headers.getHeaders(),
          this.getInfoFormUploadTask()
        )
        .subscribe({
          next: (data: ApiResponseRegisterTaskDetailI) => {
            ...
            this.router.navigateByUrl(
              'therapist/home/dashboard/tasks/my-tasks'
            );
          },
          error: (error) => {
            this.spinnerStatus = true;
      ...
      this.showToastError(
        'Error',
        'Debe seleccionar al menos un video para la tarea'
      );
    }
  }
